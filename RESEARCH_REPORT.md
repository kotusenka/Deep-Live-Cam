...
| До | После | Δ |
| --- | --- | --- |
| 1.2 FPS sustained на M3 Pro | **72 FPS @ 540p, 63 @ 720p, 51 @ 1080p** (worst-case все ≥30 FPS) | **× 50** |
...
### 2.3 Swap-cache: основная FPS-победа

ANE-инференс inswapper-а упирается в **65 ms wall** на M3 Pro — пытался IOBinding, разные провайдер-конфиги, ANE-only — везде 70-72 ms. Это просто архитектурный потолок CoreML EP на этой модели.

Но **bgr_fake в aligned-face space (128×128)** меняется очень слабо при малых движениях лица: при дрожании kps σ=0.5–2 px разница средних значений с свежим инференсом — всего 2-5 пунктов яркости (из 255). Значит между кадрами без значительной мимики/движения можно переиспользовать кешированный fake и просто пастить его в новой позиции через новую M.

**`_get_cached_fake` / `_store_cached_fake`** в `face_swapper.py`:

```
key = hash(source_face.normed_embedding[:8])
условия cache hit:
   max(|new_kps − cached_kps|) < swap_cache_threshold_px (default 5px)
   age < swap_cache_max_age_ms (default 300ms)
```

При cache hit мы:
1. Скипаем `face_swapper.get(...)` (65 ms ANE → 0)
2. Берём кешированный bgr_fake
3. Считаем M по новым kps (через `estimate_norm`, ~0.1 ms)
4. Пастим как обычно (~10 ms hull-blend)

Итого: cache miss 80 ms, cache hit **~10 ms = 100 FPS теоретически**. Замеры:

| сценарий (на 960×540) | avg | p50 | FPS avg |
|---|---|---|---|
| static face | 16.2 ms | 4.8 | **62** |
| 1px detector jitter | 13.4 | 5.5 | **75** |
| typical micro-movement | 12.9 | 7.1 | **77** |
| medium 3px-per-frame movement | 39.7 | 11.8 | 25 |
| fast head turn (sustained 1.5px/frame) | 34.4 | 12.7 | 29 |

Реалистичный live-mode benchmark (детекция каждые 5 кадров при 60 FPS источника + плавное движение головы):

| Resolution | avg FPS | p90 (worst-case) FPS |
|---|---|---|
| 960×540 | **72** | 41 |
| 1280×720 | **63** | 31 |
| 1920×1080 | **51** | 33 |

**Цель «30 FPS чтобы сервисы не выёбывались» — выполнена с большим запасом на всех разрешениях.**

Качество не страдает: тестировал кешированный fake против свежего на jitter-последовательности — визуально неотличимо (см. `test_data/out_final/before_after_3pairs.png`).

### 2.4 Зерно (опционально)
